...
void update_sigma_current(double *sigma_current, double *mi_current, double *sample_current, unsigned int matrix_index, unsigned int columns){
    if(matrix_index == 0){
        *sigma_current = 0.0;
        return;
    }
...
double *online_eccentricity_s(
    ...
    *eccentricity = (1/(matrix_index + 1.0)) + (dot_product/denominator);
    return matrix_aux;
}
