...
TEST_CASE("Two dimensional micro cluster grouping test")
{
    ...
    adjency_matrix(micro_clusters_arr, adj_nodes, n_micro, TWO_DIMENSION_COLS);
    macro_clusters_arr = bfs_grouping(macro_clusters_arr, micro_clusters_arr, adj_nodes, number_of_macro_clusters, n_micro);
    printf("Amount of macro clusters ::: %u \n", n_macro);
    REQUIRE(true == true);
